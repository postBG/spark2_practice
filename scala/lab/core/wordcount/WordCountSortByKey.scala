 ...
 *   1.2 개수가 큰 순서(내림차순)로 정렬 => OrderedRDDFunctions.sortByKey(ascending: Boolean = true, numPartitions: Int = self.partitions.length)
 *   
 * 2. IDE에서 local 실행....
 ...
 *   3.3 run : CentOS7-14 SSH 연결(MobaXterm) > # cd /kikang/spark-2.0.0-bin-hadoop2.7 > # ./bin/spark-submit --master spark://CentOS7-14:7077 --class lab.core.wordcount.WordCountSortByKey ./dev/app/Spark2.0.0_Edu_Lab-0.0.1-SNAPSHOT-jar-with-dependencies.jar README.md
 * 
 */
object WordCountSortByKey { 
  ...
  def main(args: Array[String]): Unit = {
    ...
    val conf=new SparkConf().setAppName("WordCountSortByKey")
    ...
    //--개수(Key)로 내림차순 정렬....
    val sortedRDD = swappedRDD.sortByKey(false)
