 ...
 *   1.2 개수가 큰 순서(내림차순)로 10개만 출력 => RDD.top(num: Int)(implicit ord: Ordering[T]): Array[T]
 *   
 * 2. IDE에서 local 실행....
 ...
 *   3.3 run : CentOS7-14 SSH 연결(MobaXterm) > # cd /kikang/spark-2.0.0-bin-hadoop2.7 > # ./bin/spark-submit --master spark://CentOS7-14:7077 --class lab.core.wordcount.WordCountTopOrdering ./dev/app/Spark2.0.0_Edu_Lab-0.0.1-SNAPSHOT-jar-with-dependencies.jar README.md
 * 
 */
object WordCountTopOrdering { 
  ...
  def main(args: Array[String]): Unit = {
    ...
    val conf=new SparkConf().setAppName("WordCountTopOrdering")
    ...
    //--Ordering 객체 생성....
    val secondValueOrdering = new Ordering[(String, Int)] {
    ...
    reducedRDD.top(10)(secondValueOrdering).foreach { x => println("word_count(10 only) : " + x) }
